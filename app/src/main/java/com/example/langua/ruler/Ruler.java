...
import com.example.langua.activities.testSound;
import com.example.langua.activities.testTranslate;
...
public class Ruler {
    ...
    public static final String cardId           = "cardId";                                         // name of card's ID extra in Intent
    public static final String practiceState    = "practiceState";                                  // name of cardStudyState extra in Intent
    public static final String sqlIndex         = "sqlIndex";                                       // name of index extra in Intent
    ...
    private TransportSQLInterface transportSQL;
    ...
    /////// LIFECYCLE //////////////////////////////////////////////////////////////////////////////
    public Ruler(TransportSQLInterface transportSQL){                                               // generate Ruler with tranportSQL
        this.transportSQL = transportSQL;
        this.index = transportSQL.getIndex();
    }
    public Ruler(Context context, int index){                                                       // generate Ruler with tranportSQL index
        this.transportSQL = MainTransportSQL.getTransport(index, context);
        this.context = context;
        this.index = transportSQL.getIndex();
    }
    public TransportSQLInterface getDatabase(){
        return transportSQL;
    }
    public void closeDatabase() {                                                                   // необходимо закрывать базы данных каждый раз при завершении работы
        if (transportSQL != null)
    ...
    public static boolean isDailyPlanCompleted(Context context){                                            // true - если учебный план завершен
        return (//getTodayLeftPhraseologyRepeatCardQuantityPreference(context) <= 0 &&
                //getTodayLeftPhraseologyStudyCardQuantityPreference(context) <= 0 &&
                getTodayLeftVocabularyStudyCardQuantityPreference(context) <= 0 &&
                getTodayLeftVocabularyRepeatCardQuantityPreference(context) <= 0);
    }
    ...
    /////// GET CARD ///////////////////////////////////////////////////////////////////////////////
    public Card getCard(String id) {                                                                // получить карту по ее айдишнику из текущей ситуационной базы данных
        if (dayState == consts.dayState.study)
            return transportSQL.getStringFromStudy(id);
        else if (dayState == consts.dayState.repeat)
            return transportSQL.getStringFromRepeat(id);
        else{
            return transportSQL.getStringFromPractiseById(id);
        }
    }
    public Card getTodayCard(Context context) {                                                     // получить первую в очереди карту, которую надо повторить сегодня - берет из текущей базы данных
        Card card;
        card = transportSQL.getTodayString(getDay(context));
        return card;
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public void reloadErrorStringWhileStudy(Card card) {                                            // кладет карту с ошибкой назаж
        ...
        if (Ruler.dayState == consts.dayState.study)
            transportSQL.reloadStudyString(card);
        else
            transportSQL.reloadRepeatString(card);
    }
    public void reloadRightStringWhileStudy(Card card) {                                            // закрывает карту и кладет ее назаж
        if (Ruler.dayState == consts.dayState.study) {
            transportSQL.reloadStudyString(card);
            transportSQL.reduceLeftStudyCount();
        } else {
            transportSQL.reloadRepeatString(card);
            transportSQL.reduceLeftRepeatCount();
        }
    }
    public void putErrorPracticeCardBack(Context context, consts.cardState state, Card card) {      // закидывает в таблицу карточку, практику которой пользователь запорол
        ...
        if (state == consts.cardState.stepTrain) {
    ...
    /////// GET CONTEXT ////////////////////////////////////////////////////////////////////////////
    private Fragment setNewFragment(){                                                              // вернуть контекст карты

        switch (getSQLIndex()){
            case VOCABULARY_INDEX:
                return new MainActivity();
            case PHRASEOLOGY_INDEX:
                return new cardPhraseology();
            default:
                return new DayPlanFragment();
        }
    }
    private Fragment setNewFragment(int trainLevel) {                                               // вернуть контекст теста
        switch (getSQLIndex()) {
            case VOCABULARY_INDEX:
                switch (trainLevel) {
            ...
            case PHRASEOLOGY_INDEX:
                switch (trainLevel){
                    case consts.P_TRANSLATE_LEARN:
                        return null;
                    case consts.P_TRANSLATE_NATIVE:
                        return null;
                    case consts.P_WRITING:
                        return null;
                    case consts.P_SENTENCE:
                        return null;
                }
        }
        return null;
    ...
    */
    public Fragment changeActivityWhileStudy(Context lastContext) {                                  // для пользования во время изучения и повторения
        ...
        Fragment fr;
            ...
            if (card == null){
                Log.i("main", "There is no card");
                if (Ruler.dayState == consts.dayState.study)
                    transportSQL.returnTodayStudyDatabase();
                else
                    transportSQL.returnTodayRepeatDatabase();
                Ruler.dayState = null;
                return new DayPlanFragment();
            }

            if (card.getRepeatlevel() >= consts.START_TEST &&
                    card.getPracticeLevel() > (card.getRepeatlevel()) &&
                    (card.getRepeatlevel() <= CardRepeatManage.getEndTest(getSQLIndex()))) {
                ...
                cardStudyState = consts.cardState.problemTrain;
                    ...
                    card.getRepeatlevel() < consts.START_TEST) {
                cardStudyState = consts.cardState.problemTrain;
            } else if (card.getRepeatlevel() > CardRepeatManage.getEndTest(getSQLIndex())) {        // all trains was passed
                cardStudyState = consts.cardState.problemTrain;
            } else {
                cardStudyState = consts.cardState.study;
            }

            // короче, мне лень делать все нормально
            // просто если показатель больше предложения, то показывай карточку
            //if (card.getRepeatlevel() > consts.END_TEST)
            //    cardStudyState = consts.cardState.study;
            // короче, я чуть чуть поменяла, на пару строчек выше оно просто показывает тренировку
            // с максимальным уровенм

            fr = null;
            if (cardStudyState == consts.cardState.problemTrain) {
                fr = setNewFragment(card.getPracticeLevel());
            } else if (cardStudyState == consts.cardState.stepTrain) {
                fr = setNewFragment(card.getRepeatlevel());
            } else {
                fr = setNewFragment();
            }
    ...
    public Fragment closeTest(Context context, boolean isRight, consts.cardState getState, Card card)
        ...
        if (getState == consts.cardState.stepTrain) {
            if (card.getPracticeLevel() != card.getRepeatlevel()) {
                isProblemTrain = true;
                return setNewFragment(card.getPracticeLevel());
            } else {
                isProblemTrain = false;
    ...
    public Fragment changeActivityWhilePracticeForTheNextTests(Fragment lastFragment) {             // переходит к новой активности во время практики в режиме "подготовка"
        return getFragment(transportSQL.getStringFromPractiseByLevel(getPractiseTestTypes()));
    }
    ...
    public Fragment changeActivityWhilePracticeForTheAllCards(Fragment lastFragment) {              // переход к новой активности во время практики режима "все карты"
        return getFragment(transportSQL.getStringFromPractise());
    }
    ...
    private Fragment getFragment(Card card) {                                                       // get only intent for practice
        if (card == null) {
            ...
            lastCard = card;
            return setNewFragment(getPractiseTestTypes() + consts.START_TEST - 1);
        }
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public Fragment skipPracticeNextTest(Fragment fragment, String id) {                                // выкидывает карту из таблицы практики
        transportSQL.deleteStringFromPractice(id);
    ...
    public void putCardWhilePracticeForTheNextTest(Card card, boolean isRight) {                    // кладет ошибочную карту обратно в таблицу
        transportSQL.deleteStringFromPractice(card.getId());
        if (!isRight) {
            transportSQL.addStringPractice(card);
        }
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public static void setTodayTables(Context context) {                                            // set Today tables in start of day - reload this - id tables isn't empty and fill them again
        ...
        if (Ruler.getDay(context) > transportPreferences.getLastDayComming(context)) {              // user didn't open the app this day
            ...
            transportPreferences.setTodayLeftPhraseologyStudyCardQuantityPreference(context,0);
            transportPreferences.setTodayLeftPhraseologyRepeatCardQuantityPreference(context,0);
            transportPreferences.setTodayLeftVocabularyStudyCardQuantityPreference(context,0);
            transportPreferences.setTodayLeftVocabularyRepeatCardQuantityPreference(context,0);
    ...
    private void reloadTodayTables(){                                                               // перезагружает базы данных обучения/повторения

        Log.i("main", "FULL");

        //if (transportPreferences.getTodayStudyReturned(context) < consts.DATABASE_LOAD_COMPLETED) {  // check - if study data didn't reloaded back (didn't start or did have an error)
            transportSQL.returnTodayStudyDatabase();
        //}
        //if (transportPreferences.getTodayRepeatReturned(context) < consts.DATABASE_LOAD_COMPLETED) { // check - if repeat data didn't reloaded back (didn't start or did have an error)
            transportSQL.returnTodayRepeatDatabase();
        //}
        ...
        transportSQL.fillTodayRepeatDatabase();

        if (transportSQL.getRepeatCardCount() >=
                transportPreferences.getCriticalValue(context, transportSQL.getIndex())) {
        //if (transportSQL.getRepeatCardCount() >= 14){
            Log.i("main", "bla?");
            ...
            stat.putCommand(new Command() {
                ...
                public void drop(Context context) {
                    TransportSQLInterface transportSql =
                            MainTransportSQL.getTransport(transportSQL.getIndex(), context);
                    transportSql.fillTodayStudyDatabase();
                    transportSql.closeDatabases();
                }
                public String getName(){
                    return context.getResources().getString(R.string.stat_skip_learning) + " " +
                            ActivitiesUtils.getSectionUserString(context, transportSQL.getIndex());
                }
            });
        ...
        transportPreferences.setTodayLeftVocabularyStudyCardQuantityPreference(context,transportSQL.getStudyCardCount());
        transportPreferences.setTodayLeftVocabularyRepeatCardQuantityPreference(context,transportSQL.getRepeatCardCount());


        Log.i("mainCount", Integer.toString(transportSQL.getStudyCardCount()));
        transportSQL.closeDatabases();
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public ArrayList<Integer> fillPracticeDBForNextTest(){
        return transportSQL.fillPracticeDBForNextTest();
    }
    public int fillPracticeDBForExam(){
        return transportSQL.fillPracticeDBForExams();
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////
